...
module.exports = function(app, models, io, logger) {
	...
	app.get('/target-event/indexes', function(req,res){
		if (logger.DO_LOG) {
			logger.info("Request for list of indexes for Target Event");
		}
		
		targetEventService.getIndexes(function(indexes) {
			if (!indexes) {
				responseHandler.send500(res, "Error getting indexes of Target Event");
			} else {
				res.jsonp(indexes);
				res.end();
			}
	...
	app.get('/target-event/dates', function(req,res){
		if (logger.DO_LOG) {
			logger.info("Request for list of dates for Target Event");
		}
		
		targetEventService.findDates(function(dates) {
			if (!dates) {
				responseHandler.send500(res, "Error getting dates of Target Event");
			} else {
				res.jsonp(dates);
				res.end();
			}
	...
	/**
	 * Update a Target Event with specified id
	 */
	...
	/**
	 * Delete a single Target Event with specified id
	 */
